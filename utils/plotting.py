...
def plot_km_curve(df_sv, strata, threshold, direction):
    ...
    plt.title(f'Kaplan-Meier Survival Curve: Event = Score {threshold} and {direction}')
...
def plot_interactive_first_vs_last(df_sf, df, strata):
    st.title('First vs last score comparison per person')

    # Add jitter to scores
    df_sf['score_first_jitter'] = add_jitter(df_sf['score_first'])
    df_sf['score_last_jitter'] = add_jitter(df_sf['score_last'])

    # Create the plot
    fig = px.scatter(
        df_sf, 
        x='score_first_jitter', 
        y='score_last_jitter', 
        color=strata,
        hover_data=['GP2ID'],
...
def plot_interactive_visit_month(df, outcome, strata):
    st.title('Visit Month vs. Hoehn and Yahr Stage')
